...
def load_vqgan_model(config_path, checkpoint_path):
    ...
    if config.model.target == 'tamingtransformers.taming.models.vqgan.VQModel':
        model = vqgan.VQModel(**config.model.params)
        model.eval().requires_grad_(False)
        model.init_from_ckpt(checkpoint_path)
    elif config.model.target == 'tamingtransformers.taming.models.vqgan.GumbelVQ':
        model = vqgan.GumbelVQ(**config.model.params)
        model.eval().requires_grad_(False)
        model.init_from_ckpt(checkpoint_path)
        gumbel = True
    elif config.model.target == 'tamingtransformers.taming.models.cond_transformer.Net2NetTransformer':
...
def ascend_txt(z, model, perceptor, normalize, make_cutouts, args, pMs):
    global i
    out = synth(z, model)
...
def train(i, opt, z, model, perceptor, normalize, make_cutouts, args, pMs, z_min, z_max):
    ...
    lossAll = ascend_txt(z, model, perceptor, normalize, make_cutouts, args, pMs)
